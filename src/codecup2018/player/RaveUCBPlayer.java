...
public class RaveUCBPlayer extends StandardPlayer {

    public static double UCB_PARAMETER = 0.1;
    public static int HEURISTIC_WEIGHT = 200;
    public static int MOVE_AVERAGE_WEIGHT = 200;
    ...
    private final long[] totalMoveScore = new long[32 * 64];
    private final int[] numMoveVisits = new int[32 * 64];

    public RaveUCBPlayer(String name, Evaluator evaluator, MoveGenerator generator, int expansions) {
    ...
    protected int selectMove() {
        ...
        for (int i = 0; i < expansions; i++) {
            ...
            ////DEBUG
            if (lineOfPlay[lopIndex].moves.length == 0) {
                // Endgame
                if (!board.isGameInEndGame()) {
                    System.err.println("ERROR: No moves, but not in endgame!");
                }
            } else if (board.isGameInEndGame()) {
                System.err.println("ERROR: Endgame, but node has moves!");
            }
        ...
        // Clear (TODO: try reducing instead)
        Arrays.fill(totalMoveScore, 0);
        Arrays.fill(numMoveVisits, 0);
    ...
    private class GameTreeNode {
        ...
        public GameTreeNode(int move, boolean player1, int[] moves) {
            this.move = Board.clearEval(move);
        ...
        public double getPriority(double sqrtLognVisitsParent) {
            // TODO: optimize
            double moveAverage = (numMoveVisits[move] == 0 ? 0 : totalMoveScore[move] / (double) numMoveVisits[move]);
            double ucbValue = totalScore / nVisits;
            double explorationValue = sqrtLognVisitsParent / Math.sqrt(nVisits);

            double moveAverageWeight = (nVisits > MOVE_AVERAGE_WEIGHT ? 0 : (MOVE_AVERAGE_WEIGHT - nVisits) / (double) MOVE_AVERAGE_WEIGHT);
            double ucbWeight = 1;
            double explorationWeight = UCB_PARAMETER;

            return moveAverageWeight * moveAverage + ucbWeight * ucbValue + explorationWeight * explorationValue;
        }
        ...
        public void update(int score) {
            if (nVisits == 0) {
                ...
                nVisits = HEURISTIC_WEIGHT;
                totalScore = HEURISTIC_WEIGHT * SCORE_SCALING_FACTOR * (player1 ? -score : score);
            } else {
                nVisits++;
